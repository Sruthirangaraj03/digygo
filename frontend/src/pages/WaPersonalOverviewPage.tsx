  ...
  const body = (
    <div className="min-w-0 flex-1">
      ...
      <h3 className={cn('font-bold text-[24px] leading-tight tracking-tight', accent ? 'text-white' : 'text-[#1c1410]')}>
        {typeof value === 'number' && label.includes('Rate') ? `${value}%` : value.toLocaleString()}
      </h3>
...
export default function WaPersonalOverviewPage() {
  ...
  // Period change → reset page, reload all
  useEffect(() => {
    setLogPage(0);
    fetchMain(period);
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-black/5 p-5 shadow-sm">
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={210}>
              <ComposedChart data={volume.map((v) => ({ ...v, date: fmtVolumeDate(v.date) }))}
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-black/5 text-[10px] font-bold uppercase tracking-wider text-[#9a8a7a]">
                  ...
                  <th className="px-3 py-2.5 text-left w-64">Message</th>
                    ...
                    <td className="px-3 py-3">
                      {row.sender === 'agent' && row.status ? (
                        <span className={cn(
                          ...
                          row.status === 'sent'      ? 'bg-[#f0ebe5] text-[#9a8a7a]'   :
